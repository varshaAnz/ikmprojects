...
//				break;
	...
	private String getMessageTemplate(String notificationCode, String localizationMessage) {
		...
		try {
			...
			//System.out.print("localizationMessage##########################"+localizationMessage);
			message = ((ArrayList<String>) messageObj).size()!=0?((ArrayList<String>) messageObj).get(0):"Dear {1}, Your {2} application is created with reference number{3}. You can login to the portal using your mobile number provided in the application to check the current status of the application. Portal URL - https://ksmart.lsgkerala.gov.in/digit-ui/citizen/login - KSMART IKMLSG";
		} catch (Exception e) {
...
//			customizedMsg = customizedMsg.replace(NOTIF_OWNER_NAME_KEY, entryset.getValue());
	...
	public void sendWhatsapp(List<SMSRequest> smsRequestList, boolean isSMSEnabled) {
		if (isSMSEnabled) {
			if (CollectionUtils.isEmpty(smsRequestList))
				log.info("Messages from localization couldn't be fetched!");
			for (SMSRequest smsRequest : smsRequestList) {
				producer.push(config.getWhatsappNotifTopic(), smsRequest);
				log.info("SMS SENT!");
			}
		}
	}
